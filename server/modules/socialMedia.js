  ...
  try {
    ...
    if (!pageId) {
      try {
        ...
        // Smart detection: If user has business permissions AND has pages, treat as business page
        // But if they have business permissions but no pages, they're still a personal account
        if (hasBusinessPermissions) {
          console.log(`[${new Date().toISOString()}] User has business permissions - checking if they have actual pages`);
          
          // Check if user actually has pages (not just permissions)
          try {
            const pagesResponse = await axios.get('https://graph.facebook.com/v18.0/me/accounts', {
              params: {
                access_token: accessToken,
            ...
            if (pagesResponse.data.data && pagesResponse.data.data.length > 0) {
              // User has actual pages - treat as business page
              ...
              isPersonalAccount = false;
              pageDetectionMethod = 'business_permissions_with_pages';
              
              console.log(`[${new Date().toISOString()}] Facebook Business Page detected via business permissions with pages: id=${pageId}, name=${pageName}`);
            } else {
              // User has business permissions but no pages - treat as personal account
              console.log(`[${new Date().toISOString()}] User has business permissions but no pages - treating as personal account`);
              pageId = userId;
              pageName = userName;
              pageAccessToken = accessToken; // For personal accounts, user token is the page token
              isPersonalAccount = true;
              pageDetectionMethod = 'personal_account_with_business_permissions';
              
              console.log(`[${new Date().toISOString()}] Facebook Personal Account with business permissions: id=${pageId}, name=${pageName}`);
            }
          } catch (pagesError) {
            console.log(`[${new Date().toISOString()}] Could not check pages list, treating as personal account:`, pagesError.response?.data || pagesError.message);
            pageId = userId;
            pageName = userName;
            pageAccessToken = accessToken; // For personal accounts, user token is the page token
            isPersonalAccount = true;
            ...
            console.log(`[${new Date().toISOString()}] Facebook Personal Account (fallback): id=${pageId}, name=${pageName}`);
          }
...
router.post(['/send-dm-reply/:userId', '/api/send-dm-reply/:userId'], async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      // Handle the "outside of allowed window" error
      if (dmError.response?.data?.error?.code === 10 && 
          dmError.response?.data?.error?.error_subcode === 2534022) {
            
        console.log(`[${new Date().toISOString()}] Instagram DM sent outside of allowed window for user ${sender_id}.`);
        
        // Check if we're currently within the allowed time window
        const isWithinWindow = isWithinInstagramTimeWindow();
        const nextWindow = getNextInstagramTimeWindow();
        
        // Update original message status with time restriction error
        const messageKey = `InstagramEvents/${userId}/${message_id}.json`;
        try {
          const getCommand = new GetObjectCommand({
            Bucket: 'tasks',
          ...
          if (isWithinWindow) {
            // If we're within the window but still getting this error, mark as handled
            messageData.status = 'handled';
            messageData.error = 'Outside of allowed window';
            messageData.error_details = 'Instagram restricts DM sending to specific time windows';
            messageData.updated_at = new Date().toISOString();
            ...
            console.log(`[${new Date().toISOString()}] Updated DM status to handled (time restriction) at ${messageKey}`);
            
            return res.json({ 
              success: true, 
              warning: 'Message marked as handled but DM not sent: outside of Instagram\'s allowed time window',
              handled: true,
              code: 'TIME_RESTRICTION',
              details: 'Instagram restricts when messages can be sent. Try again during business hours.'
            });
          } else {
            // If we're outside the window, mark for retry later
            messageData.status = 'pending_retry';
            messageData.error = 'Outside of allowed window';
            messageData.error_details = 'Message will be retried during business hours';
            messageData.retry_after = nextWindow.toISOString();
            messageData.updated_at = new Date().toISOString();
            ...
            console.log(`[${new Date().toISOString()}] Updated DM status to pending_retry at ${messageKey}`);
            
            return res.status(429).json({ 
              error: 'Instagram time restriction', 
              code: 'TIME_RESTRICTION',
              details: 'Instagram restricts DM sending to specific time windows. Message will be retried during business hours.',
              retry_after: Math.floor((nextWindow.getTime() - Date.now()) / 1000), // Seconds until next window
        ...
        return res.status(429).json({ 
          error: 'Instagram time restriction', 
          code: 'TIME_RESTRICTION',
          details: 'Instagram restricts DM sending to specific time windows. Please try again during business hours.',
  ...
  try {
    ...
    try {
      ...
      console.log(`[${new Date().toISOString()}] Found connection data for ${userId}:`, { 
        ...
        tokenLength: connectionData.access_token ? connectionData.access_token.length : 0,
        tokenPrefix: connectionData.access_token ? connectionData.access_token.substring(0, 20) + '...' : 'none'
      });
      ...
      if (connectionData.access_token && connectionData.facebook_page_id) {
        ...
        if (!token || typeof token !== 'string' || token.length < 50) {
          console.error(`[${new Date().toISOString()}] ❌ INVALID TOKEN FORMAT for ${userId}:`, {
            tokenType: typeof token,
            tokenLength: token ? token.length : 0,
            tokenPreview: token ? token.substring(0, 50) + '...' : 'null'
          });
          
          // Try to get a fresh token using user token if available
          if (connectionData.user_access_token) {
            console.log(`[${new Date().toISOString()}] 🔄 Attempting to get fresh page token using user token...`);
            try {
              const freshPageToken = await getValidPageToken(connectionData.user_access_token, connectionData.facebook_page_id);
              if (freshPageToken) {
                console.log(`[${new Date().toISOString()}] ✅ Fresh page token obtained, updating connection...`);
                
                // Update the connection with the fresh token
                ...
                const putCommand = new PutObjectCommand({
                  Bucket: 'tasks',
                  Key: connectionKey,
                  Body: JSON.stringify(updatedConnectionData, null, 2),
                  ContentType: 'application/json',
                });
                
                await s3Client.send(putCommand);
                console.log(`[${new Date().toISOString()}] ✅ Connection updated with fresh token`);
                
                // Return the updated token data
                return {
                  access_token: freshPageToken,
                  user_access_token: connectionData.user_access_token,
              ...
              console.error(`[${new Date().toISOString()}] ❌ Failed to get fresh page token:`, refreshError.response?.data || refreshError.message);
            }
          }
          
          // If we can't get a fresh token, return null to trigger re-authentication
          console.error(`[${new Date().toISOString()}] ❌ Cannot proceed with invalid token for ${userId}`);
          return null;
        }
...
async function sendFacebookDMReply(userId, senderId, text, messageId) {
  try {
    ...
    if (!tokenData) {
      ...
      throw new Error('No Facebook token found for user - please re-authenticate with Facebook');
    }
    ...
    console.log(`[${new Date().toISOString()}] Sending DM using ${tokenData.is_personal_account ? 'user ID' : 'page ID'}: ${pageId}, token type: ${tokenData.is_personal_account ? 'user' : 'page'}`);

    // Send the message via Facebook Messenger API
    // For personal accounts, use the user ID directly
    // For business pages, use the page ID
    const endpointId = tokenData.is_personal_account ? tokenData.user_id : pageId;
    
    console.log(`[${new Date().toISOString()}] Using endpoint ID: ${endpointId} for ${tokenData.is_personal_account ? 'personal account' : 'business page'}`);
    
    const response = await axios.post(`https://graph.facebook.com/v19.0/${endpointId}/messages`, {
...
router.post(['/webhook/facebook', '/api/webhook/facebook'], async (req, res) => {
  ...
  // CRITICAL FIX: Log the actual payload to debug the structure
  console.log(`[${new Date().toISOString()}] 🔍 FACEBOOK WEBHOOK PAYLOAD DEBUG:`);
  ...
  console.log(`[${new Date().toISOString()}] Body type:`, typeof body);
  console.log(`[${new Date().toISOString()}] Has object property:`, body && typeof body === 'object');
  console.log(`[${new Date().toISOString()}] Object keys:`, body ? Object.keys(body) : 'No body');
      ...
      try {
        ...
        if (Contents) {
            ...
            if (obj.Key.endsWith('/token.json')) {
              try {
                ...
                if (token.page_id === webhookPageId || token.user_id === webhookPageId) {
                  ...
                  firebaseUserId = keyParts[1]; // Extract Firebase user ID from key path
                  
                  // CRITICAL FIX: Use Facebook user ID for storage (this is what frontend uses)
                  // The frontend sends Facebook user ID, so we need to store with that ID
                  storeUserId = token.user_id;
                  ...
                  console.log(`[${new Date().toISOString()}] ✅ STORING UNDER FACEBOOK USER ID: ${storeUserId} (this is what frontend uses)`);
          ...
          if (!matchedToken) {
            ...
            // Strategy 2: Try to find by connection data only
            try {
              const connectionListCommand = new ListObjectsV2Command({
                Bucket: 'tasks',
                Prefix: `FacebookConnection/`,
                  ...
                  if (obj.Key.endsWith('/connection.json')) {
                    try {
                      ...
                      if (connection.facebook_page_id === webhookPageId || connection.facebook_user_id === webhookPageId) {
                        ...
                        firebaseUserId = obj.Key.split('/')[1];
                        storeUserId = connection.facebook_user_id;
                        console.log(`[${new Date().toISOString()}] ✅ Found Facebook connection for webhook Page ID ${webhookPageId}: firebase_user_id=${firebaseUserId}, facebook_user_id=${connection.facebook_user_id}`);
                        console.log(`[${new Date().toISOString()}] ✅ STORING UNDER FACEBOOK USER ID: ${storeUserId} (this is what frontend uses)`);
      ...
      if (Array.isArray(entry.messaging)) {
        for (const msg of entry.messaging) {
          try {
            ...
            // CRITICAL FIX: Broadcast real-time update to frontend
            const broadcastData = {
            ...
            // Broadcast to all connected clients for this user (Facebook user ID)
            const broadcastResult = broadcastUpdate(storeUserId, broadcastData);
            console.log(`[${new Date().toISOString()}] Broadcast result for ${storeUserId}: ${broadcastResult ? 'SUCCESS' : 'NO CLIENTS'}`);
            
            // CRITICAL FIX: Also try broadcasting with Facebook page ID for frontend compatibility
            // The frontend connects to SSE using Facebook page ID, so broadcast to that too
            if (matchedToken && matchedToken.page_id && matchedToken.page_id !== storeUserId) {
              const pageBroadcastResult = broadcastUpdate(matchedToken.page_id, broadcastData);
              console.log(`[${new Date().toISOString()}] Facebook page broadcast result for ${matchedToken.page_id}: ${pageBroadcastResult ? 'SUCCESS' : 'NO CLIENTS'}`);
            }
            
            // Also try broadcasting with Firebase user ID if different
            if (firebaseUserId && firebaseUserId !== storeUserId) {
              const firebaseBroadcastResult = broadcastUpdate(firebaseUserId, broadcastData);
              console.log(`[${new Date().toISOString()}] Firebase broadcast result for ${firebaseUserId}: ${firebaseBroadcastResult ? 'SUCCESS' : 'NO CLIENTS'}`);
            }
...
// Facebook-specific SSE endpoint for real-time updates
router.get(['/events/:facebookId', '/api/events/:facebookId'], (req, res) => {
  ...
  console.log(`[${new Date().toISOString()}] Handling Facebook SSE request for /events/${facebookId} (reconnect since: ${sinceTimestamp || 'new connection'})`);
  ...
  const initialEvent = {
    type: 'connection',
    message: `Connected to Facebook events for ${facebookId}`,
    timestamp: Date.now(),
    connectionId
  };
  ...
  if (!sseClients.has(facebookId)) {
    sseClients.set(facebookId, []);
  }
  
  const clients = sseClients.get(facebookId);
  clients.push(res);
  activeConnections.set(res, Date.now());
  
  console.log(`[${new Date().toISOString()}] Facebook SSE client connected for ${facebookId}. Total clients: ${clients.length}`);
  ...
  if (sinceTimestamp > 0) {
    ...
    setImmediate(async () => {
      try {
        // Check for missed events in FacebookEvents
        const checkPaths = [`FacebookEvents/${facebookId}/`];
              ...
              for (const obj of Contents) {
                ...
                try {
                  ...
                  if (eventTime > sinceTimestamp) {
                    missedEvents.push({
                      type: event.type === 'message' ? 'facebook_message' : 'facebook_comment',
                      data: event,
                      timestamp: eventTime
                    });
                  }
        ...
        if (missedEvents.length > 0) {
          console.log(`[${new Date().toISOString()}] Sending ${missedEvents.length} missed Facebook events to SSE client for ${facebookId}`);
          ...
          for (const event of missedEvents) {
            try {
              ...
              console.error(`[${new Date().toISOString()}] Error sending missed Facebook event to SSE client:`, err.message);
          ...
          console.log(`[${new Date().toISOString()}] No missed Facebook events found for ${facebookId} since ${new Date(sinceTimestamp).toISOString()}`);
          res.write(`data: ${JSON.stringify({
            type: 'missed_events_summary',
            count: 0,
            timestamp: Date.now(),
        ...
        console.error(`[${new Date().toISOString()}] Error checking for missed Facebook events:`, error.message);
      }
    });
  }
  ...
  req.on('close', () => {
    const updatedClients = sseClients.get(facebookId)?.filter(client => client !== res) || [];
    sseClients.set(facebookId, updatedClients);
    activeConnections.delete(res);
    
    console.log(`[${new Date().toISOString()}] Facebook SSE client disconnected for ${facebookId}. Remaining clients: ${updatedClients.length}`);
    if (updatedClients.length === 0) {
      console.log(`[${new Date().toISOString()}] No more Facebook clients for ${facebookId}, cleaning up`);
      sseClients.delete(facebookId);
    }
  });
});
...
// Utility endpoint to fix corrupted Facebook connections
router.post(['/fix-facebook-connection/:userId', '/api/fix-facebook-connection/:userId'], async (req, res) => {
  setCorsHeaders(res);
  
  const { userId } = req.params;
  
  try {
    console.log(`[${new Date().toISOString()}] Attempting to fix corrupted Facebook connection for user ${userId}`);
    
    // Check if connection exists and is corrupted
    const connectionKey = `FacebookConnection/${userId}/connection.json`;
    let connectionData = null;
    ...
    try {
      ...
      console.log(`[${new Date().toISOString()}] Found connection data:`, {
        hasToken: !!connectionData.access_token,
        tokenLength: connectionData.access_token ? connectionData.access_token.length : 0,
        tokenPreview: connectionData.access_token ? connectionData.access_token.substring(0, 20) + '...' : 'none'
      });
      
      // Check if token is corrupted (too short or placeholder)
      if (connectionData.access_token && 
          (connectionData.access_token.length < 50 || 
           connectionData.access_token.includes('temp-token') ||
           connectionData.access_token.includes('EXISTING_TOKEN'))) {
        
        console.log(`[${new Date().toISOString()}] Detected corrupted token, attempting to get fresh token...`);
        
        // Try to get a fresh token using the user token if available
        if (connectionData.user_access_token) {
          try {
            const freshPageToken = await getValidPageToken(connectionData.user_access_token, connectionData.facebook_page_id);
            if (freshPageToken) {
              ...
              await s3Client.send(putCommand);
              console.log(`[${new Date().toISOString()}] Successfully fixed corrupted Facebook connection for user ${userId}`);
              
              return res.json({ 
                success: true, 
                message: 'Facebook connection fixed successfully',
                fixed: true
              });
            }
          } catch (refreshError) {
            console.error(`[${new Date().toISOString()}] Failed to refresh token:`, refreshError.message);
          }
        }
        
        // If we can't fix it, delete the corrupted connection
        console.log(`[${new Date().toISOString()}] Cannot fix corrupted connection, deleting it...`);
        const deleteCommand = new DeleteObjectCommand({
          Bucket: 'tasks',
          Key: connectionKey,
        });
        ...
        await s3Client.send(deleteCommand);
        console.log(`[${new Date().toISOString()}] Deleted corrupted Facebook connection for user ${userId}`);
        
        return res.json({ 
          success: true, 
          message: 'Corrupted Facebook connection deleted. Please reconnect.',
          deleted: true
        });
      } else {
        console.log(`[${new Date().toISOString()}] Facebook connection appears to be valid`);
        return res.json({ 
          success: true, 
          message: 'Facebook connection is already valid',
          valid: true
        });
      }
    } catch (connectionError) {
      console.log(`[${new Date().toISOString()}] No Facebook connection found for user ${userId}`);
      return res.status(404).json({ 
        error: 'No Facebook connection found',
        message: 'No connection to fix'
      });
    }
  } catch (error) {
    console.error(`[${new Date().toISOString()}] Error fixing Facebook connection for ${userId}:`, error);
    res.status(500).json({ error: 'Failed to fix Facebook connection' });
  }
});

// Utility endpoint to fix personal account detection issues
router.post(['/fix-facebook-personal-detection/:userId', '/api/fix-facebook-personal-detection/:userId'], async (req, res) => {
  setCorsHeaders(res);
  
  const { userId } = req.params;
  
  try {
    console.log(`[${new Date().toISOString()}] Attempting to fix personal account detection for user ${userId}`);
    
    // Check if token exists and is incorrectly detected as business page
    const tokenKey = `FacebookTokens/${userId}/token.json`;
    let tokenData = null;
    ...
    try {
      ...
      const data = await s3Client.send(getCommand);
      tokenData = JSON.parse(await data.Body.transformToString());
      
      console.log(`[${new Date().toISOString()}] Found token data for ${userId}:`, {
        hasToken: !!tokenData.access_token,
        pageId: tokenData.page_id,
        userId: tokenData.user_id,
        isPersonalAccount: tokenData.is_personal_account,
      ...
      // Check if this is incorrectly detected (personal account detected as business page)
      if (tokenData.access_token && 
          tokenData.page_id === tokenData.user_id &&
          !tokenData.is_personal_account) {
        
        console.log(`[${new Date().toISOString()}] Detected incorrect detection - personal account detected as business page`);
        
        // Delete the incorrectly detected token
        const deleteCommand = new DeleteObjectCommand({
          Bucket: 'tasks',
          Key: tokenKey,
        });
        ...
        console.log(`[${new Date().toISOString()}] Deleted incorrectly detected Facebook token for user ${userId}`);
        
        res.json({ 
          success: true, 
          message: 'Incorrectly detected Facebook token deleted. Please reconnect your Facebook account.',
          reason: 'personal_account_detected_as_business_page'
        });
      } else {
        res.json({ 
          success: true, 
          message: 'Token appears to be correctly detected.',
          reason: 'token_correctly_detected'
        });
      }
    } catch (error) {
      if (error.name === 'NoSuchKey' || error.$metadata?.httpStatusCode === 404) {
        res.json({ 
          success: true, 
          message: 'No Facebook token found to fix.',
    ...
    console.error(`[${new Date().toISOString()}] Error fixing Facebook personal account detection for user ${userId}:`, error);
    res.status(500).json({ 
      error: 'Failed to fix Facebook personal account detection',
      details: error.message 
    });
  }
});

// Function to retry pending Instagram messages during business hours
async function retryPendingInstagramMessages() {
  try {
    console.log(`[${new Date().toISOString()}] Checking for pending Instagram message retries...`);
    
    // Only retry during business hours
    if (!isWithinInstagramTimeWindow()) {
      console.log(`[${new Date().toISOString()}] Outside business hours, skipping retry check`);
      return;
    }
    
    const listCommand = new ListObjectsV2Command({
      Bucket: 'tasks',
      Prefix: `InstagramEvents/`,
    });
    const { Contents } = await s3Client.send(listCommand);
    ...
    for (const obj of Contents) {
      if (obj.Key.endsWith('.json') && !obj.Key.includes('reply_') && !obj.Key.includes('comment_')) {
        try {
          ...
          // Check if this is a pending retry message
          if (messageData.status === 'pending_retry' && messageData.error === 'Outside of allowed window') {
            const retryAfter = new Date(messageData.retry_after);
            const now = new Date();
            
            // Only retry if the retry time has passed
            if (now >= retryAfter) {
              console.log(`[${new Date().toISOString()}] Retrying pending Instagram message: ${obj.Key}`);
              
              // Extract user ID and message ID from the key
              const keyParts = obj.Key.split('/');
              const userId = keyParts[1];
              const messageId = keyParts[2].replace('.json', '');
              
              // Try to send the message again
              try {
                // Find token data for this user
                const tokenListCommand = new ListObjectsV2Command({
                  Bucket: 'tasks',
                  Prefix: `InstagramTokens/`,
                });
                ...
                let tokenData = null;
                if (tokenContents) {
                  for (const tokenObj of tokenContents) {
                    if (tokenObj.Key.endsWith('/token.json')) {
                      const tokenGetCommand = new GetObjectCommand({
                        Bucket: 'tasks',
                        Key: tokenObj.Key,
                      });
                      const tokenDataResponse = await s3Client.send(tokenGetCommand);
                      const token = JSON.parse(await tokenDataResponse.Body.transformToString());
                      if (token.instagram_user_id === userId) {
                        tokenData = token;
                        break;
                      }
                ...
                if (tokenData && messageData.sender_id && messageData.text) {
                  // Attempt to send the message
                  const response = await axios({
                    method: 'post',
                    url: `https://graph.instagram.com/v22.0/${tokenData.instagram_graph_id}/messages`,
                    headers: {
                      Authorization: `Bearer ${tokenData.access_token}`,
                      'Content-Type': 'application/json',
                    },
                    data: {
                      recipient: { id: messageData.sender_id },
                      message: { text: messageData.text },
                    },
                  });
                  
                  // If successful, update status to replied
                  messageData.status = 'replied';
                  messageData.updated_at = new Date().toISOString();
                  messageData.retry_successful = true;
                  ...
                  console.log(`[${new Date().toISOString()}] Successfully retried Instagram message: ${obj.Key}`);
                }
              } catch (retryError) {
                console.error(`[${new Date().toISOString()}] Error retrying Instagram message ${obj.Key}:`, retryError.response?.data || retryError.message);
                
                // If it's still a time restriction error, update the retry time
                if (retryError.response?.data?.error?.code === 10 && 
                    retryError.response?.data?.error?.error_subcode === 2534022) {
                  messageData.retry_after = getNextInstagramTimeWindow().toISOString();
                  messageData.updated_at = new Date().toISOString();
                  ...
                  console.log(`[${new Date().toISOString()}] Updated retry time for Instagram message: ${obj.Key}`);
                }
              }
            }
          }
        } catch (error) {
          console.error(`[${new Date().toISOString()}] Error processing pending retry message ${obj.Key}:`, error);
        }
      }
    }
  } catch (error) {
    console.error(`[${new Date().toISOString()}] Error in retryPendingInstagramMessages:`, error);
  }
}

// Schedule retry checks every hour
setInterval(retryPendingInstagramMessages, 60 * 60 * 1000); // Every hour

// Export getFacebookTokenData function for use in other modules
export { getFacebookTokenData };

// AI Instant Reply endpoint - proxies to RAG server
router.post(['/instant-reply', '/api/instant-reply'], async (req, res) => {
  const { username, notification } = req.body;
  
  try {
    console.log(`[${new Date().toISOString()}] [AI-INSTANT-REPLY] Processing instant reply for ${username} on ${notification?.platform || 'unknown'} platform`);
    
    // Validate required fields
    if (!username || !notification) {
      return res.status(400).json({ 
        error: 'Missing required fields: username and notification are required' 
      });
    }

    // Forward request to RAG server (same as OldServer.js)
    const ragServerUrl = 'http://localhost:3001/api/instant-reply';
    
    console.log(`[${new Date().toISOString()}] [AI-INSTANT-REPLY] Forwarding to RAG server: ${ragServerUrl}`);
    
    const ragResponse = await axios.post(ragServerUrl, {
      username,
      notification
    }, {
      headers: {
    ...
    console.log(`[${new Date().toISOString()}] [AI-INSTANT-REPLY] Successfully received response from RAG server for ${username}`);
    
    // Return the RAG server response
    res.status(ragResponse.status).json(ragResponse.data);
    
  } catch (error) {
    console.error(`[${new Date().toISOString()}] [AI-INSTANT-REPLY] Error:`, error.response?.data || error.message);
    
    // Handle specific error cases
    if (error.code === 'ECONNREFUSED') {
      return res.status(503).json({ 
        error: 'RAG server is not available. Please try again later.',
    ...
    if (error.response?.status === 429) {
      return res.status(429).json({ 
        ...
        details: error.response.data?.error || 'Too many requests'
      });
    }
    
    res.status(500).json({ 
      error: 'Failed to generate AI instant reply',
      details: error.response?.data?.error || error.message
    });
  }
});

// Utility function to check if current time is within Instagram's allowed window
function isWithinInstagramTimeWindow() {
  const now = new Date();
  const hour = now.getHours();
  const dayOfWeek = now.getDay(); // 0 = Sunday, 6 = Saturday
  
  // Instagram typically allows messaging during business hours (9 AM - 6 PM) on weekdays
  // This is a conservative estimate - actual windows may vary
...
// Utility function to get next allowed time window
function getNextInstagramTimeWindow() {
  const now = new Date();
